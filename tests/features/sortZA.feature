Feature:Products sorting
  I want to ensure that products are sorted from Z-A.
  ...
  Scenario Outline: Checking subcategories sorting from Z to A
    When I visit the endpoint "GET" "/api/c/{categoryUrlPart}?size=30&page=0&sort=name-desc"
    Then the status code of the response should be 200
    And the response time should be below 1000 milliseconds
    And products should be sorted from Z to A
